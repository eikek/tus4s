  ...
  def routes: HttpRoutes[F] = HttpRoutes.of {
    ...
    case req @ POST -> Root / UploadId(id) =>
      req.headers.get[XHttpMethodOverride].map(_.method) match
        ...
        case _            => NotFound()
    case req @ POST -> Root =>
      config.extensions.creation match
        case Some(creation) =>
          for {
            input <- req.as[CreationRequest[F]]
            // support defer-lenght?
            fail1 = Option.when(
              input.uploadLength.isEmpty && !creation.supportsDeferLength
            )(BadRequest("Upload-Defer-Length not supported"))
            // supports with-upload?
            fail2 = Option.when(input.hasContent && !creation.supportsUpload)(
            ...
            resp <- (fail1 <+> fail2 <+> fail3).fold(creation.create(input).flatMap {
    ...
    case OPTIONS -> Root =>
      NoContent
        ...
        .withExtensions(config.extensions.all)
        .withTusResumable
